...
<div class="officers view large-10 medium-8 columns content">
    <h3><?= h($officer->officer_rank->rank_name . ' ' . $officer->user->firstName . ' ' . $officer->user->lastName) ?></h3>
    <table class="vertical-table">
        <?php if($loggedUser['isAdmin']) : ?>
            <tr>
                <th scope="row"><?= __('Is Admin') ?></th>
                <td><?= $officer->user->isAdmin ? __('Yes') : __('No'); ?></td>
            </tr>
        <?php endif; ?>
        <tr>
        ...
        <tr>
            ...
            <td><?= h($officer->user->firstName) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Last Name') ?></th>
            <td><?= h($officer->user->lastName) ?></td>
        </tr>
        <tr>
